...
#' 
#' @param speciate probability each species will speciate in each time-step (0-1)
#' @param extinction probability each species will go extinct in each time-step (0-1)
#' @param time.steps number of time-steps for simulation
#' @details Simuates a phylogeny under the above parameters. There are two important things to note: (1) speciation is randomised before extinction, and only one thing can happen to a lineage per timestep - thus this isn't a fair randomisation, and (2) I'm fairly certain this code works fine, but particularly weird parameter values could well cause a crash.
...
#' @param extinction probability each species will go extinct in each time-step (0-1)
#' @param time.steps number of time-steps for simulation
#' @param tr.range vector of length two specifying boundaries for trait values (see notes); initial two species will be at the 25th and 75th percentiles of this space (which is also the boundary). See also \code{tr.wrap}
#' @param sp.tr speciation rate's interaction with the minimum distance between a species and the species most similar to it (see details)
#' @param ext.tr extinction rate's interaction with the minimum distance between a species and the species most similar to it (see details)
#' @param tr.walk at each time-step a species not undergoing speciation or extinction has its trait value drawn from a distribution centered at its current value and with a standard deviation set by this value. I.e., this is the rate of the Brownian motion trait evolution.
#' @param tr.wrap whether to force species' trait values to stay within the boundary defined by \code{tr.range}; default TRUE.
#' @details Simuates a phylogeny under the above parameters; an extension of \code{sim.bd.tree} and all the caveats of that function apply here. The extension is to include the evolution of a trait under Brownain motion (\code{tr.walk}; but see below), and to allow alteration of speciation and extinction based on this trait. Species have greater or lesser rates of speciation/extinction depending on whether they have a trait value similar to other species (\code{sp.tr}, \code{ext.tr}). When a speciation event happens, the two daughters split evenly about the ancestor's trait value, taking values half-way to whatever the nearest specie's value is.
#' @note To stop 
#' @return ape::phylo object with random tip.labels
#' @author Will Pearse
#' @examples \dontrun{
